...
states.each { |state, abbr| puts "State #{state} has an abbrieviation of #{abbr}" }
...
birthdays.each { |name, date| puts "#{name}'s birthday is on #{date}"}
...
login_statuses.each { |name, status| puts name if status == true } 
